...
Timestamp EPollPoller::poll(int timeoutMs, ChannelList *activeChannels)
{
    ...
    LOG_DEBUG("%s:%s() => 注册到EPollPoller上的channel数量，ChannelMap::size() = %lu\n", __FILE__, __FUNCTION__, channels_.size());
...
void EPollPoller::updateChannel(Channel *channel)
{
    ...
    {
        LOG_DEBUG("%s:%s:%d", __FILE__, __FUNCTION__, __LINE__);
        int fd = channel->fd();
        // 没有感兴趣事件说明可以从epoll对象中删除该channel了
        if (channel->isNoneEvent())
        {
            LOG_DEBUG("%s:%s:%d EPOLL_CTL_DEL, channel->fd() = %d", __FILE__, __FUNCTION__, __LINE__, channel->fd());
        ...
        {
            LOG_DEBUG("%s:%s:%d EPOLL_CTL_MOD, channel->fd() = %d", __FILE__, __FUNCTION__, __LINE__, channel->fd());
...
void EPollPoller::removeChannel(Channel *channel)
{
    ...
    LOG_DEBUG("%s:%s:%d remove fd = %d\n", __FILE__, __FUNCTION__, __LINE__, fd);

    int index = channel->index();
    if (index == kAdded)
    {
        LOG_DEBUG("%s:%s:%d update(EPOLL_CTL_DEL, channel) fd = %d\n", __FILE__, __FUNCTION__, __LINE__, fd);
