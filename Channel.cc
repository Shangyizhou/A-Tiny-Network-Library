...
{
    LOG_INFO("channel handleEvent revents:%d fd = %d\n", revents_, this->fd());
    
    // 对端关闭事件
    // 当TcpConnection对应Channel，通过shutdown关闭写端，epoll触发EPOLLHUP
    if ((revents_ & EPOLLHUP) && !(revents_ & EPOLLIN))
    {
        LOG_DEBUG("%s:%s:%d the client close, the fd = %d", __FILE__, __FUNCTION__, __LINE__, this->fd());
        // 确认是否拥有回调函数
        if (closeCallback_)
        {
            closeCallback_();
    ...
    {
        LOG_DEBUG("%s:%s:%d the error, the fd = %d", __FILE__, __FUNCTION__, __LINE__, this->fd());
        if (errorCallback_)
        {
            errorCallback_();
        }
    ...
    {
        LOG_DEBUG("%s:%s:%d channel have read events, the fd = %d\n", __FILE__, __FUNCTION__, __LINE__, this->fd());
        if (readCallback_)
        {
            LOG_DEBUG("%s:%s:%d channel call the readCallback_(), the fd = %d\n", __FILE__, __FUNCTION__, __LINE__, this->fd());
